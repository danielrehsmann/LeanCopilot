...
def getCt2CmakeFlags : IO (Array String) := do
  ...
  | .linux => flags := flags ++ #["-DWITH_ACCELERATE=OFF", "-DWITH_OPENBLAS=ON", "-DOPENBLAS_INCLUDE_DIR=../../OpenBLAS", "-DOPENBLAS_LIBRARY=../../OpenBLAS/libopenblas.so", "-DWITH_CUDA=ON", "-DWITH_CUDNN=ON"]
  | .windows => flags := flags
